...
import 'package:learningdart/views/child_sends_notification_parent.dart';
  ...
  Future<void> _syncParent() async {
    ...
    if (enteredParentName.isEmpty || enteredParentId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text("⚠️ Enter both Parent Name and Parent ID.")),
      );
      return;
    }
    ...
    if (_currentPosition == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content:
                Text("⚠️ Location not available. Cannot send sync request.")),
      );
      return;
    }

    // Ensure user is authenticated
    String? childId = _getCurrentUserId();
    if (childId == null) return;

    try {
      print("✅ Parent Synced Successfully!");

      // ✅ Update child document with Parent_ID
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("✅ Sync request sent successfully!")),
      );
    } catch (e) {
      print("❌ Error syncing parent: $e");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("❌ Error syncing parent: $e")),
      );
    }
  }
